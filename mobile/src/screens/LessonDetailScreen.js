        ...
        <Animated.View style={[styles.exerciseWrapper, { opacity: fadeAnim, transform: [{ translateY: slideAnim }] }]}>
          ...
          <View style={styles.contentCard}>
            ...
            {content?.examples?.map((ex, i) => (
              <View key={i} style={styles.exampleRow}><Text style={styles.izonText}>{ex.izon}</Text><Text style={styles.englishText}>{ex.english}</Text></View>
            ))}
          </View>
          {content?.culturalNotes?.map((note, i) => (
            <View key={i} style={[styles.contentCard, styles.cultureCard]}>
          ...
          <View style={styles.exerciseContainer}>
            ...
            <View style={styles.optionsContainer}>
              {exercise.options?.map((opt, idx) => (
                <TouchableOpacity 
                  key={idx} 
                  style={[styles.optionButton, String(selectedOption) === String(opt.id || idx) && styles.selectedOption]} 
    ...
    return (
      <View style={styles.container}>
        <LinearGradient colors={['#1a4c2e', '#43a047']} style={styles.summaryContainer}>
          ...
          <TouchableOpacity style={styles.retryButton} onPress={() => navigation.navigate('Lessons')}>
...
const styles = StyleSheet.create({
    ...
    contentCard: { backgroundColor: theme.card, borderRadius: 15, padding: 20, marginBottom: 15, elevation: 2 },
    ...
    englishText: { fontSize: 14, color: theme.subText },
    ...
    optionButton: { padding: 15, backgroundColor: theme.card, borderRadius: 10, borderWidth: 1, borderColor: '#e0e0e0' },
    ...
    retryButton: { backgroundColor: theme.card, paddingHorizontal: 30, paddingVertical: 12, borderRadius: 25 },
