...
export default function NotificationsScreen({ navigation }) {
  ...
  const renderSettingRow = (label, value, onToggle, icon, theme, disabled = false) => (
    <View style={styles.settingRow}>
      <View style={styles.settingRowLeft}>
        <MaterialIcons name={icon} size={20} color="#4CAF50" />
        <Text style={[styles.settingLabel, { color: theme.subText }]}>{label}</Text>
      </View>
      <Switch
        value={value}
...
const NotificationItem = React.memo(({ item, index, slideAnim, fadeAnim, handlePress, renderSwipeActions, theme }) => {
  const translateY = slideAnim.interpolate({
    inputRange: [0, 1],
    outputRange: [20, 0]
  });
  ...
  return (
    ...
    >
      <Swipeable renderRightActions={() => renderSwipeActions(item)} overshootRight={false}>
        <TouchableOpacity
          style={[styles.notifItem, { backgroundColor: theme.card }, !item.read && styles.unreadNotif]}
        ...
        >
          ...
          <View style={styles.notifContent}>
            <View style={styles.notifHeader}>
              <Text style={[styles.notifTitle, { color: theme.text }, !item.read && styles.unreadTitle]} numberOfLines={1}>
                {item.title}
              </Text>
              {!item.read && <View style={styles.unreadDot} />}
            </View>
            ...
            <View style={styles.notifFooter}>
              ...
              {item.metadata?.category && (
                <View style={[styles.notifCategory, { backgroundColor: theme.background }]}>
                  <Text style={[styles.notifCategoryText, { color: theme.subText }]}>{item.metadata.category}</Text>
                </View>
              )}
            </View>
          </View>
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <FlatList
        ...
        ListEmptyComponent={() => renderEmptyState(theme)}
      ...
      {notifications.length > 0 && (
        <Animated.View style={[styles.actionButtons, { opacity: fadeAnim }]}>
          {unreadCount > 0 && (
            <TouchableOpacity style={[styles.markAllButton, { backgroundColor: theme.card }]} onPress={markAllAsRead}>
              <MaterialIcons name="done-all" size={20} color="#4CAF50" />
              <Text style={styles.markAllText}>Mark All Read</Text>
            </TouchableOpacity>
          )}
          <TouchableOpacity style={[styles.clearAllButton, { backgroundColor: theme.card }]} onPress={clearAll}>
          ...
          <View style={[styles.modalContent, { backgroundColor: theme.card }]}>
            ...
            <ScrollView style={styles.modalBody}>
              <View style={styles.settingsSection}>
                <Text style={[styles.settingsSectionTitle, { color: theme.text }]}>Channels</Text>
                {renderSettingRow('Push Notifications', settings.channels?.push, (v) => updateSettings('channels.push', v), 'notifications-active', theme)}
                {renderSettingRow('Email Notifications', settings.channels?.email, (v) => updateSettings('channels.email', v), 'email', theme)}
                {renderSettingRow('In-App Notifications', settings.channels?.inApp, (v) => updateSettings('channels.inApp', v), 'phone-android', theme)}
              </View>

              <View style={styles.settingsSection}>
                <Text style={[styles.settingsSectionTitle, { color: theme.text }]}>Notification Types</Text>
                {renderSettingRow('Lesson Reminders', settings.types?.lessonReminders, (v) => updateSettings('types.lessonReminders', v), 'school', theme)}
                {renderSettingRow('Streak Alerts', settings.types?.streakAlerts, (v) => updateSettings('types.streakAlerts', v), 'whatshot', theme)}
                {renderSettingRow('Achievements', settings.types?.achievements, (v) => updateSettings('types.achievements', v), 'emoji-events', theme)}
                {renderSettingRow('Friend Activity', settings.types?.friendActivity, (v) => updateSettings('types.friendActivity', v), 'people', theme)}
                {renderSettingRow('New Content', settings.types?.newContent, (v) => updateSettings('types.newContent', v), 'new-releases', theme)}
                {renderSettingRow('Tips & Tricks', settings.types?.tipsAndTricks, (v) => updateSettings('types.tipsAndTricks', v), 'lightbulb', theme)}
                {renderSettingRow('Security Alerts', settings.types?.security, (v) => updateSettings('types.security', v), 'security', theme)}
              </View>

              <View style={styles.settingsSection}>
                <Text style={[styles.settingsSectionTitle, { color: theme.text }]}>Email Frequency</Text>
                <View style={styles.frequencyButtons}>
                  {['instant', 'daily', 'weekly', 'never'].map((freq) => (
                    ...
                    >
                      <Text style={[styles.frequencyText, { color: theme.subText }, settings.emailFrequency === freq && styles.activeFrequencyText]}>
...
const styles = StyleSheet.create({
  ...
  notifItem: { flexDirection: 'row', backgroundColor: '#FFFFFF', padding: 15, borderLeftWidth: 4 },
  ...
  notifCategoryText: { fontSize: 9, color: '#666666', textTransform: 'capitalize' },
  ...
  markAllButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFFFFF', paddingHorizontal: 15, paddingVertical: 10, borderRadius: 25, elevation: 5, gap: 6 },
  markAllText: { color: '#4CAF50', fontSize: 14, fontWeight: '600' },
  clearAllButton: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#FFFFFF', paddingHorizontal: 15, paddingVertical: 10, borderRadius: 25, elevation: 5, gap: 6 },
  ...
  emptyTitle: { fontSize: 20, fontWeight: 'bold', color: '#000000', marginBottom: 10 },
  emptyText: { fontSize: 14, color: '#999', textAlign: 'center', lineHeight: 20 },
  modalOverlay: { flex: 1, justifyContent: 'flex-end' },
  modalContent: { backgroundColor: '#FFFFFF', borderTopLeftRadius: 30, borderTopRightRadius: 30, maxHeight: '90%' },
  ...
  settingsSectionTitle: { fontSize: 18, fontWeight: 'bold', color: '#000000', marginBottom: 15 },
  ...
  settingLabel: { fontSize: 16, color: '#666666' },
  ...
  frequencyText: { color: '#666666', fontWeight: '600' },
  activeFrequencyText: {
    color: '#fff',
    fontWeight: '600',
  },
